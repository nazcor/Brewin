...
from type_valuev4 import Closure, Obj, Type, Value, create_value, get_printable
...
class Interpreter(InterpreterBase):
    ...
    def __call_func(self, call_ast):
        ...
        # if the function call is a method call
        if call_ast.elem_type == InterpreterBase.MCALL_DEF:
            obj_name = call_ast.get("objref")       # holds holds object's name 
            objref = self.env.get(obj_name)         # holds object

            if objref is None:
                super().error(ErrorType.NAME_ERROR, f"Variable {obj_name} not found.")
            if objref.t != Type.OBJ:
                super().error(ErrorType.TYPE_ERROR, f"Variable {obj_name} is not an object.")

            obj_env = objref.v                      # holds object's environment
            target_closure = obj_env.env.get(func_name)     # holds method
            if target_closure is None:
                # if there is no method in the current object, check proto
                proto = obj_env.env.get("proto")
                if proto is not None:
                    target_closure = self.__check_proto(proto, func_name)
            if target_closure is not None:
                # check if method is no longer a function/lambda
                if target_closure.t != Type.CLOSURE:
                    super().error(ErrorType.TYPE_ERROR, f"Function {func_name} is changed to non-function type.")
                target_closure = target_closure.value()     # holds method's definition
        else:
        ...
        # if method call, add "this" variable
        if call_ast.elem_type == InterpreterBase.MCALL_DEF:
            obj_name = call_ast.get("objref")   # holds object'sname
            objref = self.env.get(obj_name)     # holds object's environment
            new_env[InterpreterBase.THIS_DEF] = objref            # assigns "this" variable to object
    ...
    def __prepare_env_with_closed_variables(self, target_closure, temp_env):
        for var_name, value in target_closure.captured_env:
            ...
            if value.t != Type.OBJ and value.t != Type.CLOSURE:
    ...
    def __assign(self, assign_ast):
        ...
        # checks to see if assignment is an object
        parsed_var_name = var_name.split(".")
        if len(parsed_var_name) == 2:
            lhs_name = parsed_var_name[0]   # holds object name
            rhs_name = parsed_var_name[1]   # holds field/method name

            obj = self.env.get(lhs_name)
            # if no such object exists
            if obj is None:
                super().error(
                    ErrorType.NAME_ERROR, f"Object {lhs_name} not found"
                )
            # if variable is not an object
            if obj.t is not Type.OBJ:
                super().error(
                    ErrorType.TYPE_ERROR, f"Dot operator used on non-object {rhs_name}"
                )
            # check if that proto field is assigned to objects and nil only
            if rhs_name == "proto" and not(src_value_obj.t == Type.OBJ or src_value_obj.t == Type.NIL):
                super().error(
                    ErrorType.TYPE_ERROR, f"Proto field cannot be of type {src_value_obj.t}"
                )
            
            obj_value = obj.v   # holds the object's environment
            target_value_obj = obj_value.env.get(rhs_name)
            if target_value_obj is None:
                obj_value.env.set(rhs_name, src_value_obj)
            else:
    ...
    def __eval_expr(self, expr_ast):
        if expr_ast.elem_type == InterpreterBase.NIL_DEF:
            return Value(Type.NIL, Interpreter.NIL_DEF)
    ...
    def __eval_name(self, name_ast):
        ...
        parsed_var_name = var_name.split(".")
        if len(parsed_var_name) == 2:
            lhs_name = parsed_var_name[0]   # holds object's name
            rhs_name = parsed_var_name[1]   # holds field/method name
            
            obj = self.env.get(lhs_name)    # holds object
            ...
            obj_env = obj.v                 # holds object's environment
            val = obj_env.env.get(rhs_name)
            if val is not None:
                return val
            
            # if variable is not present, check proto
            proto = obj_env.env.get("proto")
            if proto is not None:
                result = self.__check_proto(proto, rhs_name)
                if result is not None:
                    return result

            closure = self.__get_func_by_name(rhs_name, None)
            if closure is not None:
                return Value(Type.CLOSURE, closure)
            
            super().error(
                ErrorType.NAME_ERROR, f"Variable/function {rhs_name} not found"
            )
    ...
    def __setup_ops(self):
        ...
        self.op_to_lambda[Type.OBJ] = {}
        self.op_to_lambda[Type.OBJ]["=="] = lambda x, y: Value(
            Type.BOOL, x.value() == y.value()
        )
        self.op_to_lambda[Type.OBJ]["!="] = lambda x, y: Value(
            Type.BOOL, x.value() != y.value()
    ...
    def __check_proto(self, obj, var_name):
        if obj.t == Type.NIL:
            return None

        obj_env = obj.v.env     # holds proto's environment

        # check for the variable in proto
        val = obj_env.get(var_name)
        # if it is not in proto, check proto's proto
        if val is None:
            proto = obj_env.get("proto")
            if proto is None:
                return None
            return self.__check_proto(proto, var_name)
